...
theme_set(theme_half_open(font_size = 12, font_family = "Arial"))
okabe_ito <- list(
  black = "#000000",
  orange = "#E69F00", 
  sky_blue = "#56B4E9",
...
# you may need a custom annotation table here
# the key headers are protein_ID (this is the protein ID used in diann, pulled from the fasta header >sp|protein_ID|etc)
# also gene_id to get a human readable name

# make annotation table or pull from an excel sample sheet. make sure to have a column for file_name which will be used to match to the data
```{r}
...
# load data
```{r}
file1 <- "/Users/jbard/Library/CloudStorage/SynologyDrive-home/repos/bardlab_diann/example_data/Pan2024_diann_report.parquet"

report_df_raw <- arrow::read_parquet(file1) |>
  as_tibble() %>%
  mutate(protein_ID = Protein.Group) %>%
  left_join(human_annotations, by = "protein_ID") |>
  # load any custom protein annotations here
  left_join(annotation_table, by = "Run")

# the key values are PG.Q.Value and PG.MaxLFQ
# PG.MaxLFQ are the estimated relative quantities for each protein across samples inferred by diann
# the Q value is the confidence metric for the value, so we filter to make sure we only have high-confidence values

report_df_filtered <- report_df_raw |>
  filter(Q.Value <= 0.01,
         Lib.Q.Value <= 0.01,
         Global.Q.Value <= 0.01,
         Lib.PG.Q.Value <= 0.01,
  ...
  select(Run, sample_id, biological_sample, strain, treatment, biorep,
         protein_ID, gene_symbol, gene_alias,
